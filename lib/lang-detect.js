...
export function detectLanguage(text) {
    ...
    if (roCharCount > 3 || roWordCount > 2)
        return 'ro';
    return 'en';
}
 ...
 */
export function detectLangFromHtml(htmlLang) {
    if (!htmlLang)
        return '';
 ...
 */
export function detectLangFromUrl(url) {
    if (!url)
        return '';
    try {
 ...
 */
export function needsTranslation(htmlLang, url, textLang) {
    // 1. Check HTML lang attribute (most reliable signal)
    const fromHtml = detectLangFromHtml(htmlLang);
    if (fromHtml && EN_RO.has(fromHtml))
        return false;
    if (fromHtml)
        return true; // known non-EN/RO language
    // 2. Check URL TLD
    const fromUrl = detectLangFromUrl(url);
    if (fromUrl && EN_RO.has(fromUrl))
        return false;
    if (fromUrl)
        return true; // known non-EN/RO language
 ...
 */
export function getSourceLang(htmlLang, url) {
    const fromHtml = detectLangFromHtml(htmlLang);
    if (fromHtml)
        return fromHtml;
    const fromUrl = detectLangFromUrl(url);
